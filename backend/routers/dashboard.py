...
def dashboard_kpis(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if all_preds:
        ...
        high_risk_count = sum(1 for p in all_preds if p.risk_band.value.upper() in ("HIGH", "CRITICAL"))
        critical_count = sum(1 for p in all_preds if p.risk_band.value.upper() == "CRITICAL")
    else:
        avg_risk_score = 0.0
